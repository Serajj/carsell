<?php
   ...
   * @copyright 2014
   * @version $Id: class_dbtools.php,v 1.00 2014-01-10 21:12:05 gewa Exp $
   */
  ...
  class dbTools
  {
      ...
      public function doBackup($fname = '', $gzip = true)
      {
          if (!($sql = $this->fetch())) {
              ...
              $ext = ($gzip ? '.sql.gz' : '.sql');
              $data['dbbackup'] = date(self::suffix) . $ext;
      ...
      public static function optimizeDb()
      {
          ...
          $display .= '<thead><tr>';
          $display .= '<th colspan="2">' . Lang::$word->BAC_REPAIRING . '... </th>';
          $display .= '<th colspan="2">' . Lang::$word->BAC_OPTIMIZING . '... </th>';
          ...
          while ($row = Registry::get("Database")->fetchrow($result2)) {
              ...
              if (!$result) {
                  ...
                  $display .= Lang::$word->BAC_STATUS . ' <i class="green icon check" data-content="' . Lang::$word->BAC_TABLE . ' ' . $table . ' ' . Lang::$word->BAC_REPAIRED . '"></i>';
              }
              $display .= '</td>';
              $display .= '<td>' . $table . '</td>';
              $display .= '<td>';
              ...
              if (!$result) {
                  ...
                  $display .= Lang::$word->BAC_STATUS . ' <i class="green icon check" data-content="' . Lang::$word->BAC_TABLE . ' ' . $table . ' ' . Lang::$word->BAC_OPTIMIZED . '"></i>';
              }
